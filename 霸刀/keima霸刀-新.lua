...
function shiziqiang(weight)
    ...
    if weight <=10 and tstate("击倒|眩晕|定身|锁足|僵直") then
        return true
    end
...
function longxianghubu(weight)
    ---龙骧虎步
    if dis()>6 or shiziqiang(weight) then
        return false
    end
    return true
end
...
function xiangwangjiding()
    ---项王击鼎
    if dis()>6 or this_player.nCurrentRage<5 then
        return false
    end
    if not statep("免控") then
        return true
    end
...
function daoxiaofengyin()
    ---刀啸风吟
    if dis()>22 or this_player.nCurrentSunEnergy<5 or shiziqiang(10) then
        return false
    end
...
function zuizhanbaishe()
    ---醉斩白蛇
    if dis()>22 or this_player.nCurrentSunEnergy<10 or shiziqiang(10)  then
        return false
    end
...
function xiumingchenshen(weight)
    ---秀明尘身
    if dis()>6  or xuexujinping(weight) or songyanzhuwu(weight) then
        return false
    end
    if this_player.nCurrentRage >=50 then
        return true
...
function xuexujinping(weight)
    ...
    if  not qinglongliuzhan() and not naoxumi() and not suijiangtian() and dis()>6 then
        return true
    end
    return false
end
...
function Main(player)
    ...
    if not isQiaqinlong and  tstatep("免控") then
        skill(16871)
        skill(16872)
        skill(16873)
        skill(16874)
