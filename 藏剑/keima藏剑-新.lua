...
function pinghuduanyue()
	---平湖断月
	if dis() >8 or GetSkillSCD("平湖断月") then
		return false
	end
	if dis()>=4 then
		return true
...
function yunfeiyuhuang(weight)
	...
	if dis()<=4 and  (tstate("站立|僵直|收招|锁足") or (state("跑动") and target.nRunSpeed<=21) or kongzhiTimeEX(1.44)) then
		return true
	end
	return false
end
...
function heguigushan()
	---鹤归孤山
	if dis()>21 or this_player.nCurrentRage<20 or tstate("冲刺") then
		return false
	end
...
function fenglaiwushan(weight)
	---风来吴山
	if dis()>6 or  this_player.nCurrentRage<50 then
		return false
	end
...
function fengchayunjing(weight)
	---峰插云景
	if dis()>6 or this_player.nCurrentRage<10  or tstatep("免推") or weight>8 or cdEX("云飞玉皇") then
		return false
	end
...
function fengchuihe()
	---风吹荷
	if HaveTalent("风吹荷")==false or cdEX("风吹荷") or dis()>6 then
		return false
	end
...
function jingtao()
	---惊涛
	if HaveTalent("惊涛")==false or cdEX("惊涛") or dis()>8 or tstatep("免控") then
		return false
	end
...
function xieliubaoshi(weight)
	---霞流宝石
	if HaveTalent("霞流宝石")==false or cdEX("霞流宝石") or dis()>5 then
		--print("霞流宝石")
		return false
	end
	-- 内功列表："洗髓经","易筋经","紫霞功","太虚剑意","花间游","离经易道","傲血战意","铁牢律","云裳心经","冰心诀","问水诀","山居剑意","毒经","补天诀","惊羽诀","天罗诡道","焚影圣诀","明尊琉璃体","铁骨衣","分山劲","笑尘诀","莫问","相知","北傲诀"

	if (weight <=10 or tmount("补天诀|离经易道|相知|云裳心经") ) and kongzhiTimeEX(2) then
		return true
	end
	if weight<=6 then
		return true
...
function tanmei()
	---探梅
	if HaveTalent("探梅")==false or dis()>15 and ttdis()>15 then
		return false
	end
...
function songshewenxia(weight)
	---松舍问霞
	if HaveTalent("松舍问霞")==false or dis()>20 or cdEX("松舍问霞") then
		return false
	end
	-- 内功列表："洗髓经","易筋经","紫霞功","太虚剑意","花间游","离经易道","傲血战意","铁牢律","云裳心经","冰心诀","问水诀","山居剑意","毒经","补天诀","惊羽诀","天罗诡道","焚影圣诀","明尊琉璃体","铁骨衣","分山劲","笑尘诀","莫问","相知","北傲诀"
	--print("tmount")
	if tmount("惊羽诀|天罗诡道|莫问|相知|凌海决")then
		...
		--	return true
		--end
		if tstate("冲刺") and his()>8 and tstatep("免控")==false then
			return true
		end
		if dis2()>8 then
			return true
...
function findTarget(outTarger)---是否排除当前目标
	...
	for k,v in ipairs(players) do	--v是玩家对象
		local weight = 10
		if IsNpc(v) ==false and IsEnemy(v) and GetDist(this_player, v) <20 and IsVisible(this_player, v) and v.IsHaveBuff(9695, 0)==false and v.IsHaveBuff(10212, 0)==false then	--如果不是我
...
function findTargetforRange(range)
	...
	for k,v in ipairs(players) do	--v是玩家对象
		local weight = 10
		if IsNpc(v) ==false and IsEnemy(v) and GetDist(this_player, v) <range and IsVisible(this_player, v) and v.IsHaveBuff(9695, 0)==false and v.IsHaveBuff(10212, 0)==false then	--如果不是我
			if objLife(v) ==0 then
				weight = weight+ 100
			end
				if objLife(v)>80 and objLife(v)<=90 then
...
function tab(weight)
	...
	if cdEX("探梅") ==false and IsEnemy(target) then  ---如果探梅没CD 并且目标是敌对
		...
		for k,v in ipairs(GetAllMember()) do
			if GetDist(this_player, v) <20 and IsVisible(this_player, v) and obj.IsHaveBuff(9695, 0)==false and obj.IsHaveBuff(10212, 0)==false then
	...
	if cdEX("探梅")and IsParty(this_player,target) then
